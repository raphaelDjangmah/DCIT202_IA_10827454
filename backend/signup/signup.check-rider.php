...
session_start();
    ...
    if(!isset($_POST['submit'])){
        header("location:../../signinsignup/courierSignup.php?error=unauthorized entry");
        exit();
    }else{
...
//SETTING SESSIONS TO BE USED IN THE SUBSEQUENT FILES
    ...
    if(!$c_fullname){
        header("location:../../signinsignup/courierSignup.php?error=10");
    }elseif(strlen($fullname)<6){
        header("location:../../signinsignup/courierSignup.php?error=101");
    }elseif(!$c_password){
        header("location:../../signinsignup/courierSignup.php?error=111");
    }elseif(strlen($password)<6){
        header("location:../../signinsignup/courierSignup.php?error=12");
    }elseif(!$c_confirm || $password!=$r_confirm){
        header("location:../../signinsignup/courierSignup.php?error=11");
    }elseif(!$c_phone){
        header("location:../../signinsignup/courierSignup.php?error=13");
    }elseif(strlen($phone) <10 || strlen($phone) >10 ){
        header("location:../../signinsignup/courierSignup.php?error=144");
    }elseif(!$c_email){
        header("location:../../signinsignup/courierSignup.php?error=15");
    }else{
    ...
    require('..\dbrelated\dbconnector.php');
        ...
        if ($connect){
            ...
            if($EMAIL_result>0){
                header("location:../../signinsignup/courierSignup.php?error=30");
            }else if($PHONE_result>0){
                header("location:../../signinsignup/courierSignup.php?error=31");
            }else{
                ...
                if($run){
                    ...
                    header("location:../../signinsignup/courierSignup.php?error=1000");
                }
